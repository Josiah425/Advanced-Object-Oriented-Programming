...
basic_tests_2() {
    ...
    size_t base = AllocatedSpace;
    {
        ...
        // More misc tests.
        {
		std::cerr << "NEW B START" << std::endl;
            SharedPtr<B> b(new B);
		std::cerr << "NEW B END" << std::endl;

		std::cerr << "NEW C START" << std::endl;
            SharedPtr<C> c(new C);
		std::cerr << "NEW C END" << std::endl;
		...
		std::cerr << "SharedPtr<A> a_base(b) Start" << std::endl;
            SharedPtr<A> a_base(b);
		std::cerr << "SharedPtr<A> a_base(b) End" << std::endl;
