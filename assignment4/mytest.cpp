...
int main(){
    ...
    for (int i = 0; i < 2; i++) {
        for (int j = 0; j < 3; j++) {
            for (int k = 0; k < 4; k++) {
                ...
                c[i][j][k] = counter;
                counter++;
		std::cout << a[i][j][k] << ' ' << b[i][j][k] << ' ' << c[i][j][k] << ' ' << counter << std::endl;
            }
        }
    }
    ...
    for (cs540::Array<int, 2, 3, 4>::FirstDimensionMajorIterator it = b.fmbegin(); it != b.fmend(); ++it) {
        std::cout << *it << std::endl;
    }

    for (cs540::Array<short, 2, 3, 4>::FirstDimensionMajorIterator it = c.fmbegin(); it != c.fmend(); ++it) {
        std::cout << *it << std::endl;
    }
